...
def dashboard_view(request):
    ...
    if metrics is None:
        ...
        risk_breakdown = list(
            base_qs.values("risk_level").annotate(count=Count("id")).order_by("-count")
        )
        sensitive_count = SensitiveDataReport.objects.filter(
            dataset__in=base_qs, total_sensitive_columns__gt=0
        ).count()
        ...
        trend = (
            base_qs.annotate(day=TruncDate("uploaded_at"))
            .values("day")
